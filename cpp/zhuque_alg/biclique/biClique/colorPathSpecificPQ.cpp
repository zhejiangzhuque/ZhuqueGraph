#include "colorPathSpecificPQ.h"
#include <chrono>
#include <random>
#include <cassert>

void colorPathSpecificPQ::approximateCountingAllVersion2(uint64_t T) {
// g->print();
...
// }
            ...
            for(int len = p; len <= maxPLength && len < minPQ && len <= p; len++) {//(len-len)-bipath
                double sumWtemp = 0.0;
                for(int j = 0; j < pU[pL]; j++) {
...
// printf("CY\n");
                ...
                if(len <= pR + 1 && len < minPQ && sumCXi[0] > 0.5)
                    ansAll += sumCXi[0] * sumWtemp / sampleSize / C[len - 1][len - 1];
