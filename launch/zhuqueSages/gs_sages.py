...
from zhuque_graph.sampler.sages_gs_sampler import AGSRandomWalkSampler
from zhuque_graph.preprocess.sages_gsgae_preprocess import gen_graph_feat, gen_edge_sim, gen_adj_list
from zhuque_graph.visualization import zhuque_visdom
from zhuque_graph.visualization import zhuque_tensorboard


def argument():
    parser = argparse.ArgumentParser()
    parser.add_argument('--model', type=str, default='SAGES')
    parser.add_argument('--data_name', type=str, default="cora", choices=['cora', 'citeseer', 'pubmed'])
    parser.add_argument('--sample', type=str, default='GS')
    parser.add_argument('--degree', type=int, default=3, help='degree of the approximation.')
...
zhuque_visdom.open_win(args.win_name)
zhuque_visdom.init_line()
zhuque_tensorboard.open_win(args.win_name)
# 开始处理有关子图采样的部分
...
if args.sample=='GS':
    ...
    loader = AGSRandomWalkSampler(data, adj_dic= adj_dic, adj_sim_dic=adj_sim_dic, measure='uniform',
                                  alpha=1, batch_size=500, walk_length=4, num_steps=4,
...
# 确定train_pos_edge_index是单数还是双数，这里是无向图，是双数
...
for epoch in range(1, 500):
    ...
    zhuque_visdom.append_line(epoch, acc)
    zhuque_tensorboard.draw_line(epoch, acc)
