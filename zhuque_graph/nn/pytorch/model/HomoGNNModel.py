...
def create_gnn_layers(in_feat_dim, hidden_feat_dim, num_layer, num_head, options):
    ...
    for i in range(num_layer):
        if options.task == "node":
            layer_output_dim = hidden_feat_dim // num_head if i != num_layer - 1 else options.classnum
        else:
...
def layer_forward(layer, x_feat, x_target_feat, sub_graph, options):
    if options.framework == "pyg":
...
class HomoGNNModel(torch.nn.Module):
    def __init__(
        ...
        self.gnn_layers = create_gnn_layers(
            in_feat_dim, hidden_feat_dim, num_layer, num_head, options
        )
    ...
    def forward(self, ids):
        ...
        for i in range(self.num_layer):
            ...
            x_feat = layer_forward(self.gnn_layers[i], x_feat, x_target_feat, sub_graph, self.options)
            if i != self.num_layer - 1:
                if self.options.framework == "dgl":
                    x_feat = x_feat.flatten(1)
                x_feat = F.relu(x_feat)
                x_feat = F.dropout(x_feat, self.options.dropout, training=self.training)
        if self.options.framework == "dgl" and self.mean_output:
