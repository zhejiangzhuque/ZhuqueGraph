...
from zhuque_graph.nn.pytorch.layer.psg_layers import *
from zhuque_graph.utils.psg_pair_loss import *
from zhuque_graph.utils.psg_sample_utils import *
...
class BaseModel(object):
    """
        ...
        *_hidden_channels : int
        ...
        train_node_emb: bool
            whether to core node embeddings based on node id
        edge_attr: tensor
            edge features
        emb_ea:
            weight matrix of edge features
        pretrain_emb: str
    ...
    def param_init(self):
        ...
        if self.emb_ea is not None:
            torch.nn.init.xavier_uniform_(self.emb_ea.weight)
    ...
    def train(self, data, split_edge, batch_size, neg_sampler_name, num_neg, adj_t):
        ...
        for perm in DataLoader(range(pos_train_edge.size(0)), batch_size, shuffle=True):
            ...
            if self.gnn_encoder_name.upper() == 'SAGE-EDGE':
                h = self.encoder(input_feat, adj_t, self.edge_attr, self.emb_ea.weight)
            else:
                h = self.encoder(input_feat, adj_t)
...
def create_gnn_layer(input_channels, hidden_channels, num_layers, dropout=0, encoder_name='SAGE'):
    ...
    elif encoder_name.upper() == 'SAGE-EDGE':
        return GraphSAGE(input_channels, hidden_channels, hidden_channels, num_layers, dropout)
    else:
