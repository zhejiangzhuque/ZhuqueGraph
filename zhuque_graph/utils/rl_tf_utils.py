...
import rl_np_utils as rl_np_utils
import six
...
def placeholders_from_networkxs(graph_nxs,
  ...
  """
  with tf.name_scope(name):
    graph = rl_np_utils.networkxs_to_graphs_tuple(graph_nxs, node_shape_hint,
                                               edge_shape_hint,
...
def concat(input_graphs, axis, name="graph_concat"):
  ...
  rl_np_utils._check_valid_sets_of_keys([gr._asdict() for gr in input_graphs])  # pylint: disable=protected-access
...
def data_dicts_to_graphs_tuple(data_dicts, name="data_dicts_to_graphs_tuple"):
  ...
  rl_np_utils._check_valid_sets_of_keys(data_dicts)  # pylint: disable=protected-access
  with tf.name_scope(name):
